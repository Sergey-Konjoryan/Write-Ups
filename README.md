# Penetration Testing Lab Reports 📑

This repository serves as a technical portfolio of my hands-on experience in vulnerability assessment and exploitation. Each report follows a structured methodology: **Reconnaissance, Enumeration, Exploitation, and Privilege Escalation.**

## Lab Machine Index:

### 🛡️ VulnHub Challenges
- [Symfonos 1](./VulnHub/symfonos_lvl1.md) - SMB Enum, LFI to RCE & Path Hijacking
- [W1R3S 1.0.1](./VulnHub/w1r3s.md) - CMS Exploitation & Hash Cracking
- [Kioptrix Level 1](./VulnHub/kioptrix_lvl1.md) - Samba & Buffer Overflow Exploitation
- [Kioptrix Level 2](./VulnHub/kioptrix_lvl2.md) - SQLi & OS Command Injection
- [Tr0ll 1](./VulnHub/tr0ll1.md) - Network Traffic Analysis & CTF Methodology
- [DC-1](./VulnHub/dc1.md) - Drupalgeddon 1 (CVE-2014-3704) SQLi & SUID find
- [Raven 1](./VulnHub/raven1.md) - PHPMailer RCE & MySQL UDF PrivEsc
- [Raven 2](./VulnHub/raven2.md) - PHPMailer RCE & MySQL UDF PrivEsc
- [Matrix 1](./VulnHub/matrix1.md) - Source Code Analysis & rbash Escape
### 🛠️ Lab Environments
- [Metasploitable 2](./Metasploitable/metasploitable2.md) - Legacy Service Exploitation

---

## 🛠️ Core Skills Demonstrated:
* **Enumeration:** Advanced use of Nmap, Gobuster, and Enum4linux.
* **Exploitation:** Manual exploitation of LFI, SQLi, and vulnerable network services.
* **PrivEsc:** Linux privilege escalation via SUID, Cron jobs, and PATH manipulation.
* **Post-Exploitation:** Hash cracking (John the Ripper) and persistent access.

---
*All tests were performed in a controlled lab environment for educational purposes only.*
